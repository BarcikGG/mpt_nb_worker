...
for unit_id, group in grouped:
    ...
    if not matching_row.empty:
        ...
        for i, row in group.iterrows():
            ...
            # Первая группа: т1
            print(i)  # Индекс текущей строки
            print(t1_indices)  # Индексы первой группы
            ...
            # Третья группа: п3 и т3
            if i in p3_t3_indices:
                for col in group_3_columns:
                    col_index = group.columns.get_loc(col)
                    if row_list[col_index] == 'нб':
                        row_list[col_index] = ' '

            # Четвертая группа: п4 и т4
            if i in p4_t4_indices:
                for col in group_4_columns:
                    col_index = group.columns.get_loc(col)
                    if row_list[col_index] == 'нб':
                        row_list[col_index] = ' '
